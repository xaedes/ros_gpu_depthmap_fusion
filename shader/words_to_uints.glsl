...
// words_to_uints
// called for each input uint containing 2 words, 
// writes those 2 word values into 2 seperate uints
...
void main()
{
    ...
    uint out_idx = global_idx * 2;
    uint in_idx = global_idx;

    uint value = in_data[in_data_offset + in_idx];

    out_data[out_data_offset + out_idx + 0] = bitfieldExtract(value, 0, 16);
    out_data[out_data_offset + out_idx + 1] = bitfieldExtract(value, 16, 16);
}
