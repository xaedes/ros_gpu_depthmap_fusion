...
public:
    ...
    void sort(const T* valuesPtr, int num)
    {
        values.resize(num);
        memcpy(values.data(), valuesPtr, sizeof(T)*num);
        sort(values);
    }
    void sort(const std::vector<T>& values)
    {
        int num = values.size();
        sortedIndices.resize(num);
        sortedValues.resize(num);
        tmpSortedIndices.resize(num);
        tmpSortedValues.resize(num);
        radixWithRedistribution<T,TUintVec4>(
            group_size,
            values,
            sortedIndices,
            sortedValues,
